...
class AuthRoutes(private val db: Database) {
    fun register(route: Route) {
        ...
        route.post("/api/v1/auth/logout") {
            authFromCall(call, db)?.let { db.revoke(it.sessionId) }
            call.respond(mapOf("ok" to true))
        }
    }
}

fun authFromCall(call: ApplicationCall, db: Database): AuthUser? {
    val token = call.request.headers["Authorization"]?.takeIf { it.startsWith("Bearer ") }?.removePrefix("Bearer ")?.trim() ?: return null
