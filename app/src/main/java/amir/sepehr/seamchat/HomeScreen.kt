...
fun SeamChatHome(onLogout: () -> Unit = {}) {
    ...
    Column(Modifier.fillMaxSize().background(Bg)) {
        Row(Modifier.fillMaxWidth().padding(20.dp), verticalAlignment = Alignment.CenterVertically) {
            ...
            IconButton(onClick = { MainScope().launch { repository.logout(); onLogout() } }) {
                Icon(Icons.Default.Logout, "Log out", tint = Color.White)
            }
        }
        LazyColumn(
            ...
            item {
                Surface(color = Glass, shape = MaterialTheme.shapes.extraLarge, modifier = Modifier.fillMaxWidth()) {
                    Row(Modifier.padding(18.dp), verticalAlignment = Alignment.CenterVertically) {
                        Box(
                            Modifier.size(52.dp).background(Brush.linearGradient(listOf(Violet, Cyan)), MaterialTheme.shapes.large),
                            contentAlignment = Alignment.Center
                        ) { Text("A", color = Color.White, fontSize = 22.sp) }
                        Spacer(Modifier.width(14.dp))
                        Column(Modifier.weight(1f)) {
                            Text("Amir", color = Color.White, fontSize = 16.sp)
                            Text("Open conversation", color = Color.White.copy(.45f), fontSize = 12.sp)
                        }
            ...
            item {
                Surface(color = Glass.copy(.75f), shape = MaterialTheme.shapes.extraLarge, modifier = Modifier.fillMaxWidth()) {
                    Column(Modifier.padding(18.dp)) {
                        ...
                        Text(
                            "Authentication, sessions, PostgreSQL persistence and WebSocket transport are connected to the self-hosted backend.",
                            color = Color.White.copy(.5f), fontSize = 12.sp, lineHeight = 18.sp
                        )
                    }
                }
            }
